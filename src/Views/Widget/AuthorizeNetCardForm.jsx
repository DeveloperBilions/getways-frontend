...
export const AuthorizeNetCardForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const cloudFunction =
        type === "charge"
          ? "authorizeNetChargeCard"
          : "authorizeNetAuthorizeCard";
      ...
      console.log(
        `✅ ${type === "charge" ? "Charge" : "Authorization"} successful:`,
        response
      );
      ...
      console.error(
        `❌ ${type === "charge" ? "Charge" : "Authorization"} failed:`,
        error
      );
      setPaymentStatus({
        status: "failed",
  ...
  return (
    <Box className="container py-4" sx={{ maxWidth: "600px", mx: "auto" }}>
      ...
      {paymentStatus?.status === "failed" && (
        <Alert severity="error" sx={{ mb: 2 }}>
          {type === "charge" ? "Payment" : "Authorization"} failed:{" "}
