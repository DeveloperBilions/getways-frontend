...
const PlayerRedeemDialog = ({ 
  ...
  const [redeemAmount, setRedeemAmount] = useState(initialRedeemAmount || "");
  const [redeemFees, setRedeemFees] = useState("");
  const [remark] = useState(initialRemark || "");
  const [loading, setLoading] = useState(false);
  const [walletId, setWalletId] = useState("");
  const [paymentMethods, setPaymentMethods] = useState({
    cashAppId: "",
  ...
  // Update redeemAmount whenever initialRedeemAmount changes
  useEffect(() => {
    setRedeemAmount(initialRedeemAmount || "");
  }, [initialRedeemAmount]);
  ...
  useEffect(() => {
    async function fetchWalletData() {
      try {
        ...
        notify("Failed to fetch wallet data.", {
          type: "error",
          anchorOrigin: { vertical: "top", horizontal: "right" }
        });
      }
  ...
  const handleSubmit = async () => {
    ...
    if (redeemAmount < 15) {
      notify("Redeem amount cannot be less than 15.", { 
        type: "error",
        anchorOrigin: { vertical: "top", horizontal: "right" }
      });
      onClose();
