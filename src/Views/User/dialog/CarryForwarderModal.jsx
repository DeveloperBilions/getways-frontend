...
export default function CarryForwarderModal({
  open,
  onClose,
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      const endAtISO = toEndOfDayUTC(endDate);
      const res = await fetchAccountingSummaryV2(
        selectedType,
        selectedEntity.id,
        startAtISO,
        endAtISO,
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      downloadTextFile(
        csv,
        `accounting_${selectedType}_${name}_${startDate}_${endDate}.csv`
      );
    } catch (e) {
      ...
      <DialogContent>
        ...
        {!selectedType || !selectedEntity ? (
          <Typography variant="body2" sx={{ color: "text.secondary", mt: 2 }}>
            Choose <strong>Type</strong> and then pick an{" "}
            <strong>{selectedType || "entity"}</strong> to continue.
          </Typography>
        ) : (
          <>
            <Box
            ...
            <Typography>
              <strong>
                {selectedType === "agent" ? "Agent" : "Master Agent"}:
              </strong>{" "}
            ...
            {loading ? (
              ...
              <Box sx={{ display: "grid", gap: 1.2 }}>
                <Typography>
                  ...
                  {totals.totalRecharges.toFixed(2)}
                </Typography>
                <Typography>
                  <strong>Total Redeems:</strong>{" "}
                  {totals.totalRedeems.toFixed(2)}
                </Typography>
                <Typography>
                ...
                <Typography>
                  <strong>Paid Ticket Amount:</strong>{" "}
                  {summary.periodPayments.toFixed(2)}
                </Typography>
  ...
  function validateCommission(val) {
    ...
    if (n < 0 || n > 1000)
      return setCommissionError("Commission must be between 0 and 1000.");
    setCommissionError("");
  }
}
...
function mapUser(u) {
  ...
  const balance = Number(u.get("balance")) || 0;
  return {
    id: u.id,
    name,
    username,
...
export async function fetchEntitiesFromDB(type, search = "") {
  ...
  q.limit(50).ascending("name").select(["name", "username", "balance"]);
