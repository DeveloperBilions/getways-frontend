...
export default function CashoutMethodsDialog({ open, onClose }) {
  ...
  const fetchMethods = async () => {
    const objs = await new Parse.Query("CashoutMethod")
      .ascending("createdAt")
    ...
    setMethods(userDefined);
    setSelectedMethod((prev) => prev || userDefined[0]);
  };

  const fetchAllowedIds = async (methodName) => {
    const q = new Parse.Query("Settings");
    q.equalTo("type", `allowedCashoutAgentsFor_${methodName.toLowerCase()}`);
  ...
  const saveAllowedIds = async (methodName, ids) => {
    ...
    if (!obj) {
      ...
      obj = new Settings();
      obj.set("type", `allowedCashoutAgentsFor_${methodName.toLowerCase()}`);
    }
  ...
  const createMethod = async () => {
    ...
    try {
      const existingMethodQuery = new Parse.Query("CashoutMethod");
      existingMethodQuery.equalTo("nameLower", methodNameLower);
      const duplicate = await existingMethodQuery.first({ useMasterKey: true });

      if (duplicate) {
        setMethodSuccessMsg(`Cashout method "${methodNameTrimmed}" already exists.`);
        setTimeout(() => setMethodSuccessMsg(""), 3000);
        return;
      }

      const CashoutMethod = Parse.Object.extend("CashoutMethod");
      const method = new CashoutMethod();
      method.set("name", methodNameTrimmed);
      method.set("nameLower", methodNameLower);
      ...
      const settingsKey = `allowedCashoutAgentsFor_${methodNameLower}`;
      ...
      setMethodSuccessMsg(
        `Cashout method "${methodNameTrimmed}" created and applied to all agents!`
      );
      setTimeout(() => setMethodSuccessMsg(""), 3000);
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      agents.forEach((a) => {
        if (a.allowed) updatedAllowedSet.add(a.id);
        else updatedAllowedSet.delete(a.id);
      });

      await saveAllowedIds(selectedMethod.name, Array.from(updatedAllowedSet));
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      ...
      <DialogContent dividers>
        ...
        <Box display="flex" gap={2} mb={2}>
          ...
          <TextField
            size="small"
            placeholder="New cashout method"
            value={newMethodName}
            ...
            <Table size="small">
              <TableHead>
                <TableRow>
                  ...
                  <TableCell align="center">Cashout Allowed</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {agents.map((a) => (
